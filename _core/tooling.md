...
| markitdown | intake: pdf/office → markdown | if that deposit type | `python -c "import markitdown"` | `pip install markitdown` |
| youtube-transcript | intake: video → transcript | if that deposit type | `python -c "import youtube_transcript_api"` | `pip install youtube-transcript-api` |
| article-extractor | intake: web article → markdown | if that deposit type | `python -c "import trafilatura"` | MISSING-ASK (candidate `pip install trafilatura`) |
...
3. A MISSING tool whose install is unknown / needs auth / needs a clone → `MISSING-ASK`, never guessed.
