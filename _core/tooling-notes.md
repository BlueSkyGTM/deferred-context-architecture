# _core/tooling-notes.md: Machine-Specific Install & Troubleshooting (NOT law)

Per-machine operational notes for tools in the universal manifest. The scan + manifest live in
`_core/tooling.md`; this file only records the gotchas that cost real time to rediscover. Nothing
here changes policy.
...
carries the DB password; fetch it per-machine from the provider (e.g. Supabase to Connect to Session
