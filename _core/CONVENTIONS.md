...
Layer 1  silo/CONTEXT.md       "Where do I go?"          task to stage routing table  (~300 tok)
...
- **One-way references.** A silo reads down and outward only: silo to `../../vault` (the well),
  silo to `../../_core`, silo to `../../meta-seams`. **Never silo to another silo. Never well to silo.**
